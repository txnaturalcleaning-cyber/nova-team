...
fastify.post('/elevenlabs-inbound', async (req, res) => {
  ...
  try {
    ...
    if (key) {
      ...
      if (d?.companyName) { cfg = { ...cfg, ...d }; console.log('Config:', cfg.companyName, '| enabled:', cfg.enabled); }
    }
  } catch(e) {}

  // ✅ AI DISABLED — play simple voicemail, no ElevenLabs
  if (cfg.enabled === false) {
    console.log('AI DISABLED — playing voicemail message');
    ...
    const msg   = cfg.language === 'ru'
      ...
      : `Thank you for calling ${cfg.companyName}. Our agent was unable to answer. Please call back or leave a message after the tone.`;
    res.header('Content-Type', 'text/xml');
